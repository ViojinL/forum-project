...
// POST to mark a comment as a violation and deduct credit score
export async function POST(
  req: NextRequest,
  context: { params: { id: string } }
) {
  try {
    console.log('收到标记违规请求，评论ID:', context.params.id);
    ...
    // 查询该管理员是否已经标记过这个评论
    const existingViolation = await prisma.commentViolation.findUnique({
      where: {
        commentId_adminId: {
          commentId: id,
          adminId: adminId,
    ...
    const comment = await prisma.comment.findUnique({
      where: { id: id },
      include: { author: true, violations: true, post: true }
    });
    ...
    console.log('找到评论:', comment.id, comment.content.substring(0, 30));
    ...
    try {
      const result = await prisma.$transaction([
        // 1. 创建违规标记记录
        prisma.commentViolation.create({
          data: {
            commentId: id,
            adminId: adminId,
            reason: reason,
        ...
        prisma.userInbox.create({
          data: {
            ...
            message: `您在《${comment.post.title}》下的评论因${reason}被标记为违规，扣除${pointsDeducted}信用分。${newCreditScore < 80 ? '由于信用分低于80，您将被暂时禁言24小时。' : ''}`,
    ...
    console.error("标记违规评论失败:", error);
    return NextResponse.json(
      { error: `标记违规评论失败: ${(error as Error).message || '请稍后再试'}` },
      { status: 500 }
    );
  }
} 